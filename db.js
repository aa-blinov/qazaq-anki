...
function ensureSchema(d) {
  ...
  d.exec(`
    ...
    -- health_writes: single-row table the /api/health endpoint
    -- UPSERTs into on every probe. Lets the probe verify writes
    -- (not just reads) and gives an external monitor a single
    -- point to query for "last successful write timestamp".
...
/**
 * Fire-and-forget write that does NOT wait for persistence.
 *
 * Used by the /api/health probe: it UPSERTs one row every
 * request. We don't want the probe to wait for the SQLite
 * write-ahead log to flush to disk — that would make a slow
 * disk look like an unhealthy service. The flush happens on
 * the next idle tick, which is fine for the "is the DB
 * accepting writes" question.
 */
...
/**
 * Force a WAL checkpoint (TRUNCATE) so the WAL file doesn't
 * grow forever. Safe to call while readers/writers are active —
 * sql.js hands off to the host process, which writes a fresh
 * snapshot to disk and resets the WAL.
 *
 * Called from the backup container (POST /api/admin/checkpoint)
 * right before snapshotting, so the snapshot is a coherent
 * point-in-time view rather than "the main DB file plus an
 * open WAL". Also nice to call after large imports.
 */
export async function walCheckpoint() {
  await persist();
  // The actual CHECKPOINT happens implicitly when sql.js
  // exports — persist() writes the full snapshot to disk,
  // which is exactly what the backup container wants. So this
  // helper is mostly a marker that says "we're done writing".
  return { ok: true };
}
